...
static NSTimeInterval       g_adjustDate = 0;
+ (void)initStandardDate:(NSTimeInterval)serverDate
{
    g_adjustDate =  serverDate - [[NSDate date] timeIntervalSince1970];
//    NSLog(@"server adjustDate=%f", g_adjustDate);
}

+ (NSDate *)standardDate//:(NSDate *)date
{
    return [NSDate dateWithTimeIntervalSinceNow:g_adjustDate];
}
...
{
    NSDate *date = [NSDate standardDate];//[NSDate date];
    
    NSCalendar *calendar = [NSCalendar currentCalendar];
    
    NSDateComponents *comps = [calendar components: NSEraCalendarUnit |NSYearCalendarUnit | NSMonthCalendarUnit| NSDayCalendarUnit| NSHourCalendarUnit | NSMinuteCalendarUnit fromDate: date];
    
    [comps setHour:oclock];
    [comps setMinute:00];
    [comps setSecond:00];
...
{
    NSDate *date = [NSDate dateWithTimeInterval:60*60*24 sinceDate:[NSDate standardDate]];
...
{
    NSDate *date = [NSDate dateWithTimeInterval:0 sinceDate:newDate];
...
{
    return [NSDate stringFromDate:self withFormat:kSD_DATE_FORMAT];
//    return [self dateFromString:dateStr withFormat:kSD_DATE_FORMAT];
}
...
- (NSString *)stringDaysAgoAgainstMidnight:(BOOL)flag {
	...
	switch (daysAgo) {
		...
		case 1:
			...
			break;
		default:
			text = [NSString stringWithFormat:@"%d天前", (int)daysAgo];//days ago
	}
	return text;
}
...
+ (NSDate *)dateFromString1:(NSString *)string {
	return [NSDate dateFromString:string withFormat:[NSDate dbFormatString1]];
}
...
+ (NSString *)timestampFormatString1 {
...
// preserving for compatibility
+ (NSString *)dbFormatString1 {
	return [NSDate timestampFormatString1];
}
