...
/*加密方法*/
+ (NSString *)AES256EncryptWithPlainTextExt:(NSString *)plain {
    ...
    // san add ->
    char bytes[16] = {0};
    memcpy(bytes, [[NSString generalKey] cStringUsingEncoding:NSUTF8StringEncoding], 16);
    // add <-
    
	size_t numBytesEncrypted = 0;
    
	CCCryptorStatus cryptStatus = CCCrypt(kCCEncrypt, kCCAlgorithmAES128,kCCOptionPKCS7Padding | kCCOptionECBMode,
                                          bytes, kCCKeySizeAES128,
...
/*解密方法*/
+ (NSString *)AES256DecryptWithCiphertextExt:(NSString *)ciphertexts{
...
{
    if (string == nil)
        [NSException raise:NSInvalidArgumentException format:@"Exception in dataWithBase64EncodedString"];
    if ([string length] == 0)
        return [NSData data];
