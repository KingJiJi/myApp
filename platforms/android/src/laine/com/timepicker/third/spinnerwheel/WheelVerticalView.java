...
//        this(context, attrs, R.attr.abstractWheelViewStyle);
    ...
    protected void initAttributes(AttributeSet attrs, int defStyle) {
        ...
        TypedArray a = getContext().obtainStyledAttributes(attrs, getStyleableArryId("WheelVerticalView"), defStyle, 0);
        mSelectionDividerHeight = a.getDimensionPixelSize(getStyleId("WheelVerticalView_selectionDividerHeight"), DEF_SELECTION_DIVIDER_SIZE);
        a.recycle();
    }
    ...
    private int[] getAttrableArryId(String styleableName){
        try {
            Class<?> loadClass = getContext().getClassLoader().loadClass(getContext().getPackageName() + ".R");
            Class<?>[] classes = loadClass.getClasses();
            for(int i=0 ;i<classes.length ;i++){
